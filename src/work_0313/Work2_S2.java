	...
	public class Work2_S2 {
		public static void main(String[] args) {
			...
			while(true) {
				...
				if(mid.length() < 4 || mid.length() > 20) sw = 1; // 4자 미만이거나 20자 초과이면 sw = 1
				else {
					for(int i=0; i<mid.length(); i++) {
						if(!temp.contains(String.valueOf(mid.charAt(i)))) { // mid를 한글자씩 꺼내 temp와 비교했을 때 없으면 sw = 1
							// contains 는 문자열을 비교하는 함수라 valueOf로 char -> String 으로 변경
							sw = 1;
							break;
						}
					}
